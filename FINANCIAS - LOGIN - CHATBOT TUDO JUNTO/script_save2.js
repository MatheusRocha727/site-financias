...
//   <script src="./Finanças Residenciais save2_files/chart.umd.min.js.baixados"></script>
    ...
    // ===== Poupança: cálculo do tempo necessário para atingir a meta =====
    document.getElementById('btnCalcularPoupanca')?.addEventListener('click', ()=>{
      ...
      const rendimentoPct = parsePercent(document.getElementById('rendimentoMes')?.value);

      if (!meta || aporte <= 0 || rendimentoPct < 0) {
        document.getElementById('poupancaResultado').textContent = 'Informe meta, aporte e rendimento válidos.';
        return;
      }

      let saldo = 0;
      let meses = 0;
      const maxMeses = 1200; // limite de segurança (~100 anos)
      while (saldo < meta && meses < maxMeses) {
        // aplica rendimento mensal sobre saldo
        saldo = saldo * (1 + rendimentoPct);
        // adiciona aporte ao final do mês
        saldo += aporte;
        meses += 1;
      }

      if (meses >= maxMeses) {
        document.getElementById('poupancaResultado').textContent = 'Meta muito distante (>=100 anos). Ajuste aporte/juros.';
        return;
      }

      const anos = Math.floor(meses/12);
      const mesesRest = meses % 12;
      const resumo = `${meses} meses (${anos} anos e ${mesesRest} meses) — Valor final: ${fmtBRL(saldo)}`;
      document.getElementById('poupancaResultado').textContent = resumo;
    });

    // ===== Empréstimo: Tabela Price (parcela fixa) =====
    document.getElementById('btnCalcularEmprestimo')?.addEventListener('click', ()=>{
      const valor = parseMoney(document.getElementById('loanValor')?.value);
      const jurosPct = parsePercent(document.getElementById('loanJurosMes')?.value);
      const meses = parseInt(document.getElementById('loanMeses')?.value || '0', 10);

      if (!valor || meses <= 0) {
        document.getElementById('loanParcela').textContent = 'R$ 0,00';
        document.getElementById('loanTotalPago').textContent = 'R$ 0,00';
        document.getElementById('loanJurosTotais').textContent = 'R$ 0,00';
        return;
      }

      const i = jurosPct; // taxa mensal (ex.: 0.02 para 2%)
      let parcela = 0;
      if (i === 0) {
        parcela = valor / meses;
      } else {
      ...
      document.getElementById('loanParcela').textContent = fmtBRL(parcela);
      document.getElementById('loanTotalPago').textContent = fmtBRL(totalPago);
      document.getElementById('loanJurosTotais').textContent = fmtBRL(jurosTotais);
    });
